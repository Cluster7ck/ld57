...
func _ready() -> void:
	var fogs = []
	var fog_variants_path = "res://Scenes/background/star/"
	...
	for p in fogPoints:
		...
		instance.position = p
		star_parent.add_child(instance)
		var starscale = rng.randf_range(1, 2)
		instance.scale = Vector2(starscale, starscale)
